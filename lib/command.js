...
function validate(argv) {
  ...
  var unknownArgs = _.chain(_.keys(argv)).without('_').difference(_.values(ARGS)).value();
...
function getFreqTable() {
  if (!_.isUndefined(argv[ARGS.freqTable]) && !_.isString(argv[ARGS.freqTable])) {
    throw new Error(
      'Invalid value for argument --' + ARGS.freqTable +
      ': ' + argv[ARGS.freqTable]
    );
  }
...
function getNum() {
  if (
    ...
    throw new Error(
      'Value of --' + ARGS.num + ' must be a positive integer, ' +
      argv[ARGS.num] + ' given'
    );
  }
