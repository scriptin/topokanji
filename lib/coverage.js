...
var mean = {
  ...
  harmonic: function (values) {
    var sumOfReversed = _.chain(values)
      .map(function (n) {
        return 1 / n;
      })
      .sum()
...
function sort(freqTables, dependencies, meanType, asc) {
  ...
  return _.chain(
    buildFullTable(mergedTable, freqTables, dependencies, meanType),
  )
    .sortBy(function (row) {
...
function getCandidates(coverageData, kanjiData, candidatesCount, removing) {
  ...
  return _.chain(coverageData)
    .filter(function (row) {
      ...
      var inList = _.contains(list, char);
      if (removing) {
        return inList && !kanjiData[char].isRadical;
      } else {
        return !inList;
      }
    })
