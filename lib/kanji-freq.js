...
function mergeFreqTables(freqTables, scale) {
  ...
  _.each(freqTables, function (freqTable, freqTableIndex) {
    var row;
    var scaleFactor = scale ? freqTablesScaleFactors[freqTableIndex] : 1;
    for (var i = 1; i < freqTable.length; i += 1) {
      row = freqTable[i];
      charOccurs[row[0]] = (charOccurs[row[0]] || 0) + Math.round(scaleFactor * row[1]);
    }
  });

  var charsFreqTable = _.chain(Object.keys(charOccurs))
    .map(function (char) {
